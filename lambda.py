...
def lambda_handler(event, context):
   ...
   # s3_path = client.get_secret_value(SecretId='s3_path')

   dn_cred = json.loads(db['SecretString'])
   # s3_loc = json.loads(s3_path['SecretString'])

#establishing the connection
   conn = psycopg2.connect(
          database=dn_cred['dbname'],
          user=dn_cred['username'],
          password=dn_cred['password'],
          host=dn_cred['host'],
          port=dn_cred['port']
         )
   ...
   sql_query_conn = """with t1 as (select count(*) as total_count from pg_stat_activity where state in ('idle','active') and usename not like '%rds%') select usename,state,client_addr,count(*) as db_connectioncount_peruser,t1.total_count as db_total_count FROM pg_stat_activity a,t1 where 1=1 and state in ('idle','active') and usename not like '%rds%' group by usename,state,db_total_count,client_addr  order by db_connectioncount_peruser desc"""
...
#Executing an sql function using the execute() method
   ...
   df1 = pd.DataFrame(data1,columns=['usename','state','client_addr','db_connectioncount_peruser','db_total_count'])
   ...
   #html = '<html> <body> <h2>Top 20 Long Running Queries</h2> <table>' + table + '</table> </body> </html>' + '<html> <body> <h2>Prod Connections</h2> <table>' + table1 + '</table> </body> </html>'
   html = '<html> <body> <h2>Top 20 Long Running Queries (>5 Secs)</h2>' + table + '<h2>Prod Connections</h2>' + table1 + '</body> </html>'
